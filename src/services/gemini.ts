...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o áudio para o português do Brasil. Seja natural e preciso na transcrição. Mantenha a pontuação adequada e divida o texto em paragráfos quando for apropriado.',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateEmbeddings(text: string) {
  ...
  if (!response.embeddings?.[0].values) {
    throw new Error('Não foi fossível gerar os embeddings.')
  }
